...
export default function Projects() {
  ...
  const [typeSelected, setTypeSelected] = useState("fullStack");
  ...
  useEffect(() => {
    const projectsFilter = projectsData.filter(
      ...
      <div className="content-project relative z-[1]">
          ...
          <div className="flex items-center">
            {typeProject.map(({ type, roundedColor }) => (
              <a
                href="#"
                key={type}
                className="capitalize sm:text-base text-sm font-medium typesRamas"
                style={{ color: Themes.text }}
                onClick={(e) => handleChangeSelected(e, type)}
              >
                <div className="flex items-center">
                  <div
                    className="w-[5px] h-[5px] rounded-full mr-1"
                ...
                {typeSelected === type ? (
                  <span
                    ...
                    style={{
                      background: `linear-gradient(to right, #e700dc00, ${roundedColor} 50%, #ffffff00 100%)`,
                    }}
                  />
                ) : null}
              </a>
        ...
        <div className="min-h-[400px] flex flex-wrap justify-center w-full">
          {projectSelected.length > 0 ? (
            ...
            <div className="min-h-[300px] items-center justify-start w-full flex">
              <h4
                className="text-sm font-normal"
                style={{ color: Themes.text }}
              >
                No se han creado proyectos para esta sección aun. Es posible que
                el creador está estudiando las tecnologías para la creación de
                los mismos.
              </h4>
            </div>
          )}
